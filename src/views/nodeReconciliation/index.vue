<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="是否导出" prop="dotExport" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.dotExport == 0 ? "未导出" : "已导出"}}</span>
        </template>
      </el-table-column>
...
<script>
import { findDotWriteoffOrder , updateReconciliation } from '@/api/nodeReconciliation'
...
export default {
  ...
  methods: {
    ...
    getData(filter){
      ...
      findDotWriteoffOrder(data).then(res=>{
        // console.log(res);
        this.data = res;
        this.loading = false;
        if (res.msg == "暂无数据") {
